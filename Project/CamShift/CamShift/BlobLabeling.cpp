...
cv::Rect CBlobLabeling::DectRectangle(IplImage* image, IplImage* view) {

	CBlobLabeling blob;
	blob.SetParam(image, 1500);
	blob.DoLabeling();
	int max_width = 0;
	...
	for (int i = 0; i<blob.m_nBlobs; i++) {
		...
		if ((size.width*size.height - 10) <= max_width || max_width<(size.width*size.height)) {
			continue;
		}
		if (max_width <= (blob.m_recBlobs[i].width*blob.m_recBlobs[i].height)) {
			before = index;
			index = i;
			max_width = blob.m_recBlobs[i].width*blob.m_recBlobs[i].height;
		}
		cvRectangle(view, pt1, pt2, CvScalar(0, 0, 255));
	}
	max = cvPoint(blob.m_recBlobs[index].x, blob.m_recBlobs[index].y);
	width = cvPoint(blob.m_recBlobs[index].x + blob.m_recBlobs[index].width, blob.m_recBlobs[index].y + blob.m_recBlobs[index].height);
	cv::Rect rc;
	rc.x = max.x;
	rc.y = max.y;
	rc.width = width.x;
	...
	CvPoint p1, p2;
	cv::Rect rc2;
	p1 = cvPoint(blob.m_recBlobs[before].x, blob.m_recBlobs[before].y);
	p2 = cvPoint(blob.m_recBlobs[before].x + blob.m_recBlobs[before].width, blob.m_recBlobs[before].y + blob.m_recBlobs[before].height);
	cvRectangle(view, max, width, CvScalar(0, 255, 0));
	cvRectangle(view, p1, p2, CvScalar(255, 0, 0));//2번째로 큰 값
